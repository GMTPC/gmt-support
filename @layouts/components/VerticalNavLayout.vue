...
export default defineComponent({
  setup(props, { slots }) {
    ...
    const toggleVerticalNavCollapsed = () => { isVerticalNavCollapsed.value = !isVerticalNavCollapsed.value }
    const route = useRoute()
    const { mdAndDown } = useDisplay()
    const isClientOverlay = ref(false)

    onMounted(() => {
      // set this only on client to avoid SSR/client class mismatches
      isClientOverlay.value = mdAndDown.value
    })
